...
package object barneshut {
  ...
  extends Quad {
    val (mass, massX, massY) = (bodies.map(_.mass).sum : Float, bodies.map(b => b.x * b.mass).sum / bodies.map(_.mass).sum  : Float, bodies.map(b => b.y * b.mass).sum / bodies.map(_.mass).sum  : Float)
